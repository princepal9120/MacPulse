...
private extension Logger {
    static let fileActor = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.mactidy", category: "FileCleanupActor")
}
...
public actor FileCleanupActor {
    ...
    func cleanContents(of path: String, dryRun: Bool, progress: (@Sendable (CleanupEngineEvent) -> Void)? = nil) async throws -> (freed: Int64, item: CleanupFileItem?) {
        ...
        for item in contents {
            ...
            // Never delete the live app / test host (e.g. DerivedData/.../MacTidy.app).
